...
void loop() {
  ...
  if (millis() - printTime > 1000U) {
    ...
    voltagePH = analogPH * (VREF / ADC_RESOLUTION); // dalam Volt
    phValue = ph.readPH(voltagePH * 1000.0, temperature); // DFRobot_PH expects mV

    // === Kirim data sensor ke Supabase ===
    sendToSupabase(phValue, (int)tdsValue, turbidityVoltage);
    ...
    Serial.print("pH Tegangan : "); Serial.print(voltagePH * 1000.0, 0); Serial.println(" mV");
  ...
  // ph.calibration(voltagePH * 1000.0, temperature);
}
